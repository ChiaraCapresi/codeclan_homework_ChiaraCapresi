---
title: "R Notebook"
output: html_notebook
---

```{r}
library(tidyverse)
```
```{r}
library(CodeClanData)
```

```{r}
drinks_content
```

QUESTION 1: 

Load in the dataset and have a look at it. What are the dimensions, variable types, variable names, etc.?

```{r}
head(drinks_content)
```

```{r}
dim(drinks_content)
```

drinks_content has 242 rows and 18 columns!

```{r}
names(drinks_content)
```

These are the names of the columns

```{r}
glimpse(drinks_content)
```

These function group a series of informations about the dataframe, like name of columns , their type, their values etc.

```{r}
class(drinks_content)
```

```{r}
typeof(drinks_content)
```
The last two functions give the type of drinks_content.

```{r}
View(drinks_content)
```
 This allows us to see the dataframe.

############################


QUESTION 2:

Let’s first investigate the calories of different drinks. Select the variables Beverage_category, Beverage, Beverage prep and Calories from your data. Since we are interested in the calorie content, check if there are any NA values in the data, and drop them if there are.

```{r}
library(dplyr)
```


```{r}
drinks_content %>% 
  select(Beverage_category, Beverage, Beverage_prep, Calories)
```

These select the columns we are interested in.

```{r}
drinks_cont2<-mutate(drinks_content, Calories2= is.na(Calories), .after=Calories)
 filter(drinks_cont2, Calories2==FALSE)
```

```{r}
select(drinks_cont2, Beverage_category, Beverage, Beverage_prep, Calories)
```
#####################################

QUESTION 3:

Create a new variable (column) called calorie_diff, which stores the difference between 135 calories (135 calories = 10 cubes of sugar!) and the calories in each drink. (hint: you’ll want to subtract 135 from the calories to see which drink have more or less than 10 cups of sugar).

```{r}
drinks_content %>% 
mutate(calorie_diff= 135 - Calories, na.rm=TRUE, .after=Calories)
```
#########################

QUESTION 4:

Summarise the mean number of calories in each beverage_category. 



```{r}
drinks_content %>% 
  group_by(Beverage_category) %>% 
  summarize(mean_calories=mean(Calories, na.rm=TRUE))
```

Which 3 category of drinks have the most calories? 

```{r}
drinks_content %>% 
  group_by(Beverage_category) %>% 
  summarize(mean_calories=mean(Calories, na.rm=TRUE)) %>% 
  arrange(desc(mean_calories)) %>% 
  head(3)
```

Which 3 drinks have the least? 



```{r}
drinks_content %>% 
  group_by(Beverage_category) %>% 
  summarize(mean_calories=mean(Calories, na.rm=TRUE)) %>% 
  arrange(desc(mean_calories)) %>% 
  tail(3)
```



Write a small summary of your findings:

Smoothies, Frappuccino Blended Coffee and Signature Espresso Drinks are the three drinks with most calories in the dataframe.


Classic Espresso Drinks, Shaken Iced Beverage and coffee are the three with least calories.

########################################

QUESTION 5:


Let’s look at this a different way. What is the average number of calories in each Beverage_prep type?


```{r}
drinks_content %>% 
  group_by(Beverage_category) %>% 
  summarize(mean_calories=mean(Calories, na.rm=TRUE))
```
#######################################

QUESTION 6:

Which Beverage_prep type contains more than the average calories of all drinks?

```{r}

```


```{r}

drinks_content %>% 
  group_by(Beverage_category) %>% 
  summarize(mean_calories=mean(Calories, na.rm=TRUE))
```



#########################################

QUESTION 7:

Which is the best type of coffee drink (and by ‘drink’ we mean the combination of Beverage_category, Beverage and Beverage_prep) to get if you’re worried about consuming too many calories?

```{r}
drinks_content %>% 
  group_by(Beverage_category, Beverage, Beverage_prep) %>% 
  summarize(mean_calories=mean(Calories, na.rm=TRUE)) %>% 
  arrange(mean_calories)
```
All preparations of Tazo Tea!
